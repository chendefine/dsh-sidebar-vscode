	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			settingPathMapDesc: "DSH 路径前缀 → VSCode 容器路径前缀，格式 源=目标，多条用 ; 分隔；留空 = 不映射，直接使用原始目录（仅 workbench 与 DSH 不同容器/挂载时才需要配置）",
			settingPathMapPlaceholder: "留空 = 直接使用原始目录；示例：/data/workspace=/mnt/vscode",
			loading: "正在打开 VSCode …",
			loadHint: "长时间空白？请检查「功能设置」里的服务地址是否可达，或用「在新窗口打开」排查",
			reload: "刷新",
			openNewWindow: "在新窗口打开",
			workspace: "工作区",
			unmapped: "当前工作区路径未命中任何映射规则，已按原路径打开；若工作台看不到该目录，可在「功能设置」里配置路径映射",
		...
		const en = {
			...
			settingPathMapDesc: "DSH path prefix → VS Code container prefix, as src=dst pairs joined by \";\" ; empty = no mapping, raw paths are used as-is (only needed when the workbench runs in another container / mount)",
			settingPathMapPlaceholder: "empty = raw paths; e.g. /data/workspace=/mnt/vscode",
			...
			workspace: "Workspace",
			unmapped: "The current workspace path matched no mapping rule and was opened at its raw path; if the workbench cannot see that directory, configure the path mapping in the gear settings.",
