window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function createChatOpenRoute(deps) {
			return (path) => {
				if (!deps.takeoverEnabled() || typeof path !== "string" || path === "") return false;
				if (deps.blocked?.(path) === true) return deps.rerouteBlocked?.(path) === true;
				deps.reroute(path);
				return true;
			};
		}
		...
		*/
		var OpenRequestConsumer = class {
			constructor(deps) {
				...
				_defineProperty(this, "lastNonce", Number.NEGATIVE_INFINITY);
			}
			/**
			...
			*/
			update(request, sessionId) {
				...
				Promise.resolve(this.deps.execute(request)).catch(() => {});
			}
		};
		//#endregion
		...
		*/
		function VscodeView(props) {
			...
			if (gateRef.current === null) gateRef.current = new BootGateController({
				...
				domPaths: () => {
					try {
						...
						return Array.from(doc.querySelectorAll(".editor-group-container .tab"), (tab) => (tab.getAttribute("data-resource-name") ?? tab.querySelector(".tab-label")?.textContent ?? "").trim());
					} catch {
						return null;
					}
				},
			...
			const [gateState, setGateState] = (0, react.useState)(gate.getSnapshot());
			(0, react.useEffect)(() => gate.subscribe(() => {
				setGateState(gate.getSnapshot());
			}), [gate]);
			(0, react.useEffect)(() => () => {
				gate.dispose();
			}, [gate]);
			const bootLockRef = (0, react.useRef)(null);
			if (bootLockRef.current === null) bootLockRef.current = new WorkbenchBootLock({
			...
			const [lockSnap, setLockSnap] = (0, react.useState)(bootLock.getSnapshot());
			(0, react.useEffect)(() => bootLock.subscribe(() => {
				setLockSnap(bootLock.getSnapshot());
			}), [bootLock]);
			(0, react.useEffect)(() => () => {
				bootLock.dispose();
			}, [bootLock]);
			...
			if (openerRef.current === null) openerRef.current = createWorkbenchOpener({
				inputs: () => ({
			...
			const fenceRef = (0, react.useRef)(null);
			if (fenceRef.current === null) fenceRef.current = new FocusFenceController({
			...
			(0, react.useEffect)(() => fence.attach(), [fence]);
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(unmapped || cwdFailed) && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(NoticeRow, { text: cwdFailed ? t("cwdFailed") : t("unmapped") }),
					flash !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(NoticeRow, { text: flash }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dsh_vscodeTab_surface",
						children: [ready && bootLockHeld && bootGate.phase !== "pending" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("iframe", {
							ref: iframeRef,
							src: target,
							title: "VSCode",
						...
						}, loadKey) : null, !ready || !loaded || bootHidden ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		*/
		function isBlockedPath(path, list) {
			const name = baseNameOf$1(path.trim()).toLowerCase();
				...
				children: [
					...
					shown.map((path) => {
						...
						return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
							...
							children: [blocked ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(DocGlyph, {}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CodeGlyph, {}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: baseNameOf(path) })]
						}, path);
					}),
...
`
		...
		*/
		function installTakeovers(client, betterSidebar) {
			...
			const sessionCwd = (sessionId) => client.sessions?.list?.getSnapshot().byId?.[sessionId]?.cwd;
			const currentSessionId = () => client.sessions?.list?.getSnapshot()?.current ?? "";
			...
			if (client.inject !== void 0) client.inject(["remote.session"], (scope) => {
				...
				let disposeWrap = wrapRemoteOpenWorkspacePath(session, chatDeps);
				const reassert = () => {
					disposeWrap();
					disposeWrap = wrapRemoteOpenWorkspacePath(session, chatDeps);
				};
			...
			const stopOpenPath = workspaces === void 0 ? void 0 : wrapWorkspacesOpenPath(workspaces, chatDeps);
			const settingsDeps = {
				takeoverEnabled,
