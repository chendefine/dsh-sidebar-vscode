window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function buildVscodeUrl(base, folder) {
			const root = `${base}/`;
		...
		*/
		async function insertVscodeReferences(sessions, conversation, sessionId, refs) {
			...
			(0, react.useEffect)(() => {
				const onPaste = (event) => {
					...
					*/
					...
					if (payload !== null) {
						swallow();
						lander(sessionId, payload, fallbackOptions);
						return;
					}
...
`;
		...
		*/
		function apply(ctx) {
			...
			const lander = (sessionId, payload, options) => {
				return (async () => {
