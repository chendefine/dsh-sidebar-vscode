 ...
 */
export function createWorkbenchOpener(deps: WorkbenchOpenerDeps): WorkbenchOpener {
  const open = async (request: OpenRequest): Promise<void> => {
    ...
    // and the open channel decides existence. null only for non-absolute
    // garbage.
    ...
    // Primary channel: only tried when a workspace folder is known (it
    // addresses the extension). mapPath never returns null for a resolved
    // cwd (pass-through), so the channel is available in the no-rules
    // default too.
    if (workspace != null) {
      const capable = await deps.probeCapability(workspace)
      if (capable) {
        // Boot tag: the command names THIS workbench boot's nonce, so only
        // the host that activated with it consumes the open (see the
        // module doc — the lingering-host hole).
        const boot = deps.taggableNonce()
        const sent = await deps.sendOpenCommand({
          folder: workspace,
          path: file,
          nonce: request.nonce,
          line: request.line,
          column: request.column,
          ...(boot !== undefined && capable >= BOOT_TAG_CAP_V ? { boot } : {}),
        })
        if (sent) return
      }
    }
    // Degraded channel: reload the workbench once with VS Code web's
    // native payload parameter, stamped with the basis it was computed
    // from (ignored once that basis changes — cwd flip / settings edit).
    let authority = deps.pageHost()
    try { authority = new URL(base, deps.pageHref()).host || authority } catch { /* keep page host */ }
